...
public class AddressController extends BaseController {
    ...
    @RequestMapping("{aid}/save_aid")
    public JsonResult<Void> saveAid(@PathVariable("aid") Integer aid, HttpSession session) {
    ...
    public JsonResult<Address> getByAid(HttpSession session) {
        Address data = addressService.getByAid(getAidFromSession(session), getuidFromSession(session));
        return new JsonResult<>(OK, data);
    }
    ...
    public JsonResult<Void> changeAddress(Address address, HttpSession session) {
        ...
        Integer uid = getuidFromSession(session);
        String username = getUsernameFromSession(session);
        addressService.changeAddress(aid, uid, username, address);
        return new JsonResult<>(OK);
    }
}